...
const OFFICINA_NUMBERS = (process.env.INTERNAL_OFFICINA_NUMBERS || 'whatsapp:+393287377675')
  .split(',')
  .map(x => x.trim())
  .filter(Boolean);

const GENERAL_NUMBERS = (process.env.INTERNAL_GENERAL_NUMBERS || 'whatsapp:+393472733226,whatsapp:+393494040073')
  .split(',')
  .map(x => x.trim())
  .filter(Boolean);
...
function splitName(fullName) {
  ...
  return {
    ...
    surname: parts.slice(1).join(' ') || 'WhatsApp'
  };
}
...
function isAfterEveningCutoff(startDate) {
  return startDate && isSameDay(new Date(), startDate) && getNowDecimalHour() > 18.5;
}
...
function findAllByKey(obj, keys, out = []) {
  if (!obj || typeof obj !== 'object') return out;

  for (const key of Object.keys(obj)) {
    const val = obj[key];
    if (keys.includes(key)) out.push(val);
    if (val && typeof val === 'object') findAllByKey(val, keys, out);
  }

  return out;
}

function flattenFound(values) {
  const out = [];
...
function humanizeVehicleName(name, code) {
  ...
  const pretty = prettifyVehicleCode(upperCode);
  return pretty !== upperCode ? pretty : cleaned;
}
...
function matchVehicleAgainstUserText(vehicle, userText) {
  ...
  if (q.includes('furgone') || q.includes('van')) {
    return codeUpper.startsWith('F') || nameLower.includes('furgone') || nameLower.includes('van') || nameLower.includes('corto') || nameLower.includes('lungo');
  }
  ...
  if (q.includes('auto') || q.includes('macchina') || q.includes('vettura')) {
    return codeUpper.startsWith('A') || nameLower.includes('compact') || nameLower.includes('auto');
  }
...
function normalizeVehicleFromVehAvail(item) {
  ...
  const vehicle = core?.Vehicle || item?.Vehicle || item?.['ns1:Vehicle'] || {};
  const makeModel = vehicle?.VehMakeModel || item?.VehMakeModel || {};
  const totalCharge = core?.TotalCharge || item?.TotalCharge || {};

  const rentalRate = core?.RentalRate || item?.RentalRate || {};
  const chargesBlock = rentalRate?.VehicleCharges || core?.VehicleCharges || {};
  const vehicleCharge = safeArray(chargesBlock?.VehicleCharge || chargesBlock?.['ns1:VehicleCharge'])[0] || {};
...
function alreadyProcessedMessage(messageSid) {
  return messageSid ? processedMessageSids.has(messageSid) : false;
}
...
function alreadyProcessedFingerprint(from, body) {
  return processedMessageFingerprints.has(buildMessageFingerprint(from, body));
}
...
function resetSession(phone, profileName = 'Cliente') {
  return createSession(phone, profileName);
}
...
<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ns1="http://www.opentravel.org/OTA/2003/05">
  ...
  console.log('📥 OTA_VehAvailRateRS:', xmlText);

  if (!response.ok) throw new Error(`Errore HTTP disponibilità gestionale: ${response.status} - ${xmlText}`);
  ...
  const vehAvailsRaw = flattenFound(findAllByKey(availRs, ['VehAvail', 'ns1:VehAvail']));
  const vehicles = vehAvailsRaw
    .map(normalizeVehicleFromVehAvail)
    .filter(v => v.code && v.estimatedTotalAmount > 0);
...
async function createCarRentalReservation({ customerName, customerWhatsapp, selectedVehicle, startDate, endDate }) {
  if (!canUseCarRental()) throw new Error('Gestionale non configurato');
  ...
  const currencyCode = selectedVehicle.currencyCode || 'EUR';
...
<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ns1="http://www.opentravel.org/OTA/2003/05">
  <SOAP-ENV:Body>
    <ns1:OTA_VehResRQ>
      ...
      <VehResRQCore>
        ...
        <VehPref>
          <VehMakeModel Code="${xmlEscape(selectedVehicle.code)}" Name=""/>
        </VehPref>
        <Customer>
          <Primary>
            <PersonName>
              <GivenName>${xmlEscape(person.name)}</GivenName>
              <Surname>${xmlEscape(person.surname)}</Surname>
            </PersonName>
        ...
        <VehicleCharges>
          <VehicleCharge Purpose="1" TaxInclusive="false" IncludedInEstTotalInd="true" IncludedInRate="true" Description="${xmlEscape(selectedVehicle.chargeDescription || '')}" Amount="${vehicleChargeAmount.toFixed(2)}" CurrencyCode="${xmlEscape(currencyCode)}">
            <TaxAmounts>
              <TaxAmount CurrencyCode="${xmlEscape(currencyCode)}" Percentage="22" Total="${taxTotal.toFixed(2)}"/>
            </TaxAmounts>
          </VehicleCharge>
        </VehicleCharges>
        <TotalCharge CurrencyCode="${xmlEscape(currencyCode)}" RateTotalAmount="${rateTotalAmount.toFixed(2)}" EstimatedTotalAmount="${estimatedTotalAmount.toFixed(2)}"/>
      </VehResRQCore>
  ...
  if (!response.ok) throw new Error(`Errore HTTP prenotazione gestionale: ${response.status} - ${xmlText}`);
  ...
  const confIds = flattenFound(findAllByKey(parsed, ['ConfID', 'ns1:ConfID']));
  ...
  return {
    ...
    parsed,
    reservationStatus,
    confirmationId: conf?.['@_ID'] || uniqueId?.['@_ID'] || uniqueId?.ID || ''
  };
}
...
function isBackCommand(text) {
  return ['indietro', 'torna', 'torna indietro', 'annulla'].includes(normalize(text));
}

function isAnotherDateCommand(text) {
  return ['altra data', 'altre date', 'cambio data', 'cambiare data'].includes(normalize(text));
}
...
function getRecipients(intent) {
  return intent === 'officina' ? OFFICINA_NUMBERS : GENERAL_NUMBERS;
}
...
app.get('/nexi/cancel', (req, res) => {
  res.send('<html><head><meta charset="utf-8" /><title>Pagamento annullato</title></head><body style="font-family:Arial;padding:40px;text-align:center"><h1>Pagamento annullato</h1><p>Il pagamento non è stato completato.</p></body></html>');
});
    ...
    if (session.state === 'confirm_noleggio') {
      ...
      if (canUseNexi() && prezzoFinale > 0) {
        try {
          ...
          console.error('Errore Nexi dopo prenotazione:', error.message);
        }
      }

      await sendInternalNotification(
        GENERAL_NUMBERS,
        `✅ PRENOTAZIONE NOLEGGIO CONFERMATA\n\n` +
        `👤 ${profileName}\n` +
        `📞 ${incomingFrom}\n` +
